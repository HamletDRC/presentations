...
window.AjaxQuickDelete = {
   ...
   install: function () {
      ...
      if ((this.userRights == 'filemover' || this.userRights == 'sysop') && mw.config.get('wgNamespaceNumber') == 6) {
         ...
         if ($('#AjaxRenameLink').length) {
            $('#AjaxRenameLink')
            .append('<a href="javascript:AjaxQuickDelete.moveFile();">Move file and replace all usage</a>')
      ...
      if (this.userRights == 'sysop' && wgNamespaceNumber == 6) {
         if ($('#AjaxDupeProcess').length) $('#AjaxDupeProcess')
          .append('<a href="javascript:AjaxQuickDelete.processDupes();">Process Duplicates</a>').show();
      }
      // Define optional buttons
      if (window.QuickDeleteEnhanced && wgNamespaceNumber == 6) {
         $.each(this.insertTagButtons, function (k, v) {
            addPortletLink('p-tb', 'javascript:AjaxQuickDelete.insertTagOnPage("' + v.tag + '","' 
              + v.img_summary + '","' + v.talk_tag + '","' + v.talk_summary + '","' + v.prompt_text + '");', v.label);
         });
      }
   },
   /**
   ...
   findCreatorCB: function (result) {
      ...
      for (var id in pages) { // there should be only one, but we don't know its ID
         ...
         if (wgNamespaceNumber != 6 || !pages[id].imageinfo) {
   ...
   saveDescription: function () {
      ...
      this.savePage(page, 'Merging details from duplicate ([[' + wgPageName + ']])', 'nextTask');
   },
